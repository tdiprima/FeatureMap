...
describe('Tick Label Render Tests', function () {
  ...
  function testOrientation (orientation) {
    const sliderIndex = orientation.toLowerCase() === 'horizontal' ? 1 : 2
    ...
    it('Tick labels are rendered - ' + orientation, function () {
      ...
      const length = $(sliderId + ' .slider-tick-label').length
    ...
    it("Tick labels render inside the slider's bounds" + orientation, function () {
      ...
      const sliderRect = $(sliderId)[0].getBoundingClientRect()
      const tickLabels = $(sliderId + ' .slider-tick-label')

      for (let i = 0; i < tickLabels.length; i++) {
        const labelRect = tickLabels[i].getBoundingClientRect()
